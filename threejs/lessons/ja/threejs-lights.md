...
この記事はthree.jsについてのシリーズ記事の一つです。 最初の記事は[Three.jsの基礎知識](threejs-fundamentals.html)です。 
まだ読んでいなくて、three.jsが初めてなら、そちらから先に読んでみるといいかもしれません。
また、[設定と環境](threejs-setup.html)も読んでみてください。
[前の記事はテクスチャに関するもの](threejs-textures.html)です。

Let's go over how to use the various kinds of lights in three.
three.jsの様々な種類の光源の使い方を説明していきましょう。
...
以前に取り上げたサンプルから始めます。カメラをアップデートしましょう。
視野角を45度に、遠平面を100ユニットに、そしてカメラを原点から10ユニット上、20ユニット後ろに移動させます。
...
in our page
次に、`OrbitControls`を加えましょう。`OrbitControls`はカメラをある点の周りに回転または*旋回*させます。
`OrbitControls`はthree.jsのオプションの設定なので、まずページに含める必要があります。
...
そして、使います。`OrbitControls`に制御したいカメラと、入力イベントを取得するために使用するDOM要素を渡します。
...
target.
また、原点の5ユニット上を旋回させる対象を設定し、`controls.update`を呼びます。
そうすると、新しく設定した対象に、制御が適用されます。
...
like this
次に、ライトアップするものを作りましょう。
最初に地面を作ります。
2x2ピクセルの小さなチェッカー盤のテクスチャを、このように適用します。
